...
def view_raw_data(df):
    """" this function display 5 lines of the raw data if the user want """
    log.debug("Enter the method view_raw_data")
    start_loc=0
    end_loc=5
    is_required=True
    while is_required:
        is_required_answer=str(input("If you want to see 5 lines of raw data please enter y or n if you don't want "))
        is_required_answer=is_required_answer.lower()
        if is_required_answer!="n" and is_required_answer!='y':
            log.debug("the passed answer is\t "+is_required_answer)
            log.debug("The passed answer wasn't n or y")
            continue
        elif is_required_answer=='n':
            log.debug("The passed answer was \t"+is_required_answer)
            is_required=False
            break
        else:
            log.debug("the passed answer was \t"+is_required_answer)
            print(df.iloc[start_loc:end_loc])
            log.debug("update start and end location ")
            start_loc+=5;
            end_loc+=5
            log.debug("update start and end location "+str(start_loc)+"\t"+str(end_loc))
            row,column=df.shape
            if(start_loc>=row):
                log.debug("The new positions are greater than the number of rows in the data frame")
                print('There is no more data to be displayed')
            else:
                continue
    log.debug("Exit the method view_raw_data")
